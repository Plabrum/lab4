...
let last_class =  
  let open Studyplan in 
    ...
    | Infinite -> raise (Failure "Infinite study plan")
    | Finite x -> eval x ;;
...
module type Sequence =
  sig
    type length
    type info : {name : string; length: length; inventor: _}
    val name : string
    val length : length
    val exists : int -> bool
    val eval : int -> int option
...
module SequenceFibonacci : Sequence = Fibonacci ;;
